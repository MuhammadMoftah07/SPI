<template>
  <div class="pb-20 space-y-16 space-y-reverse">
    ...
    <section class="grid grid-cols-2 overflow-hidden">
      ...
      <img src="/imgs/flags/germany.jpg" class="w-full h-56" alt="" />
    </section>
    ...
    <section class="space-y-8">
      ...
      />
      <p class="text-sm text-theme-indigo">{{ $t("germany_liquidity_p") }}</p>
      <div class="chartsContainer">
        ...
        <aside>
          <ChartsLine
            v-if="germanyLiquidity"
            :chart-data="germanyLiquidity"
            :chart-options="lineOption"
            footerText="Average - Germany (benchmark) (b)"
            chartClass="max-h-[280px]"
          />
        </aside>
      </div>
      <ThemeLowerIdea :text="$t(`germany_liquidity_idea`)">
        <div class="grid gap-3">
          <h5 class="font-bold">{{ $t(`liquidity_ratios`) }}</h5>
          <p>
            {{ $t(`germany_liquidity_idea`) }}
          </p>
        </div>
      </ThemeLowerIdea>
    </section>
    ...
    <section class="space-y-8">
      <ThemeTitle
        ...
        :title="$t('ccc')"
      />
      <p class="text-sm text-theme-indigo">{{ $t("germany_ccc_p") }}</p>
      <div class="chartsContainer">
        <aside>
          <ChartsBar
            v-if="ksaCCC"
    ...
    <section class="space-y-8">
      ...
      <div class="chartsContainer">
        ...
        <aside>
          <ChartsLine
            v-if="germanyProfitability"
            :chart-data="germanyProfitability"
            :chart-options="lineOption2"
            footerText="Average - Germany (benchmark) (b)"
            chartClass="max-h-[280px]"
          />
      ...
      <ThemeLowerIdea>
        <div class="grid gap-3">
          ...
          <p v-for="n in 3">
            {{ $t(`germany_profitability_idea${n}`) }}
          </p>
        </div>
      </ThemeLowerIdea>
    </section>
    ...
    <section class="space-y-8">
      <ThemeTitle
        ...
        :title="$t('ROE_ROA')"
      />
      <p class="text-sm text-theme-indigo">{{ $t("germany_roe_p") }}</p>
      <div class="chartsContainer">
        <aside>
          <ChartsLine
            v-if="ksaROE"
    ...
    <section class="space-y-8">
      <ThemeTitle
        ...
        :title="$t('solvency')"
      />
      <p class="text-sm text-theme-indigo">{{ $t("germany_solvency_p") }}</p>
      <div class="chartsContainer">
        <aside>
          <ChartsLine
            v-if="ksaSolvency"
        ...
        <aside>
          <ChartsLine
            v-if="germanySolvency"
            :chart-data="germanySolvency"
            :chart-options="lineOption3"
            footerText="Average - Germany (benchmark) (b)"
            chartClass="max-h-[280px]"
          />
...
const germanyLiquidity = computed(() => {
  const data = useChartsData().liquidity_ratios_germany;
  if (!data?.length) {
    return null;
  }
  return {
...
const germanyCCC = computed(() => {
  const data = useChartsData().cash_conversion_cycle_germany;
  if (!data?.length) {
    return null;
  }
  return {
...
const germanyProfitability = computed(() => {
  const data = useChartsData().profitability_germany;
  if (!data?.length) {
    return null;
  }
  return {
...
const germanyROE = computed(() => {
  const data = useChartsData().roe_and_roa_germany;
  if (!data?.length) {
    return null;
  }
  return {
...
const germanySolvency = computed(() => {
  const data = useChartsData().solvency_ratios_germany;
  if (!data?.length) {
    return null;
  }
  return {
