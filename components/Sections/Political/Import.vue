<template>
  <div class="grid items-start grid-cols-1 mt-10 gap-14 xl:grid-cols-2">
    ...
    <ChartsBar
      ...
      footer-text="Saudi Arabia total retail imports (SAR bn), 2017A-21A(a)(b)"
    ...
    <ChartsBar
      ...
      footer-text="Saudi Arabia retail imports, by country (SAR bn), 2017-21A(a)(b)(d)(e)(f)"
      chartClass="xl:max-h-[250px] mx-auto"
      class="col-span-2 xl:col-span-1"
    />
...
const chartData = computed(() => {
  const data = useChartsData().saudi_arabia_total_imports;
  if (!data?.length) {
    return null;
  }
  return {
    ...
    datasets: [
      {
        label: "Saudi arabia total imports",
...
let chartData2 = computed(() => {
  const data = useChartsData().saudi_arabia_exports_by_country;
  if (!data || !data.length) {
    return null;
  }
  return {
